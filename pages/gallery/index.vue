...
const fetchPosts = async () => {
  ...
  try {
    const response = await fetch(
      `${apiUrl}/api/v1/posts/gallery?&search=${searchQuery.value}&per_page=${perPage}&page=${currentPage.value}`
    );
...
const {data:news , status} = await useApi("/content/gallery")
</script>
...
<template>
  <div>
    ...
    <div>
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 mb-10">
        <div
          v-for="post in posts"
          :key="post.id"
          class="border border-gray-300 rounded-lg mb-5 bg-white group overflow-hidden"
        >
        <NuxtLink :to="`/gallery/${post.slug}`" class="overflow-hidden h-48 w-full rounded-lg">
          <img
